// Scenari condivisi fra gli harness (sim.ts, analisi.ts).
import type { Medico, TurniMese, Regole } from "../src/engine/types";
import * as fs from "node:fs";

export interface ScenCfg {
  nome: string;
...
// ─── SCENARI ─────────────────────────────────────────────────────────────────
export const mediciBase = (): Medico[] => [
...
export function conAssenze(assenze: Record<number,[number,number][]>, tipo="L"): TurniMese {
  const T: TurniMese = {};
...
export function scenari(): ScenCfg[] {
  const S: ScenCfg[] = [];
